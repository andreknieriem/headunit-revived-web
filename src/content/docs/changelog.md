...
## v.3.4.0
- **Audio Sink Redesign & Dynamic Buffer Management:** Introduced an intelligent audio sink monitor (`AudioSinkHealthMonitor`) that measures queue depth and latency in real-time, automatically deepening buffers upon underrun to eliminate stutter and audio drift while preserving low latency (thanks to @o-jcardenass).
- **Dedicated Video Transport Thread & Flow Control:** Decoupled video packet processing onto its own high-priority transport thread. Video ACKs now drive responsive TCP flow-control, preventing media packet starvation and reducing wireless stutter (thanks to @o-jcardenass).
- **Video Fit Modes & Display Framing:** Replaced the legacy binary "Stretch to Fill" toggle with three distinct display fitting modes:
  - **Stretch to fill (no bars, may distort):** Stretches the projected video across the entire screen, ignoring the stream's native aspect ratio.
  - **Fit with bars (no crop, no distortion):** Preserves the original aspect ratio without distortion, placing letterbox or pillarbox bars as needed.
  - **Fill by cropping (no bars, no distortion):** Scales and centers the video to fill the screen without distortion, cleanly cropping minor outer edges (thanks to @o-jcardenass).
- **Ultrawide Display Optimization & Dynamic Scaling:** Native support for panoramic automotive displays (e.g. 1920x720, 1440x720, 1780x720). Open Headunit now announces the display's true pixel aspect ratio (`pixelAspectRatioE4`) to Android Auto and dynamically matches the visible canvas, preventing vertical 1.5x stretching and misaligned touch input (thanks to @o-jcardenass and @Sesam17).
- **Unified Touch Coordinate Mapping:** Rewrote and unified touch translation in `TouchCoordinateMapper` across all fit modes (Fill, Contain, Cover), custom insets, and ultrawide geometries.
- **Floating Launcher Overlay Button & Android Auto Exit Action:** Added a persistent, draggable floating button overlay (`TYPE_APPLICATION_OVERLAY`) allowing drivers to jump directly back to their car's OEM or custom launcher. Includes controls for button size (32dp - 120dp), opacity, and screen positioning (thanks to @Sesam17). Added a new **Android Auto Exit Action** setting (OEM Launcher, App Home, Disconnect) and fixed button visibility during reverse camera display.
- **OEM App Conflict Management (ADB Server & Auto-Kill):** Built-in internal ADB server and root backend integration to disable, kill, or restore conflicting factory projection apps (e.g. Zlink, AutoKit, SpeedPlay, Carlink) directly from Open Headunit to prevent USB/wireless hijacking.
- **BYD Steering Wheel Control Support:** Added native hardware key handling and broadcast receivers for BYD vehicle steering wheel controls and panel keys (thanks to @nicoruy).
- **Native AA & External Bluetooth Module Support:** Added direct support for external Bluetooth modules (ZBT protocol) communicating directly via vendor daemon on Chinese head units. Includes a live home screen connection status pill with auto-connect countdown, proactive notifications when Wi-Fi radio is off, and AAC audio compression optimizations for narrow wireless links (thanks to @o-jcardenass).
- **Driver Selection (Multi-Phone Management):** Intelligently manage connections when multiple paired Bluetooth phones enter the vehicle simultaneously (thanks to @andreknieriem and @o-jcardenass). Modes include **Auto** (connects to preferred or last-connected device with a configurable countdown timer), **Always Ask** (shows a driver selection dialog), or **Disabled**.
- **Theming & Splash Screen Dark Mode:** Added pure black OLED dark mode for the splash screen. Moved UI Scale setting into the Theming/Customization section to scale text and interface elements alongside custom home screen buttons.
- **Log Copy to Clipboard & Diagnostic Tools:** Added a one-tap "Copy to Clipboard" option for logs with size checks capped for Android 4 Binder safety (thanks to @peter9811). Enhanced the performance overlay with selectable corner positioning (Left / Right) to prevent OEM status bar overlap.
- **External Automation Command Surface:** Completely unified intent and broadcast control under `AutomationReceiver`. Automation apps (Tasker, MacroDroid, ADB) can now send broadcast commands (`ACTION_CONNECT`, `ACTION_DISCONNECT`, `ACTION_QUERY_STATE`, `ACTION_APPLY_SETTINGS`) without needing background activity permissions (thanks to @o-jcardenass).
- **USB AOA Switch Diagnostics & Fast-Revert Reliability:** Added diagnostic reporting for USB accessory handshake failures, support for all four AOA PIDs, asynchronous non-blocking AOA switching, and fixed connection loops with fast-reverting dongles (thanks to @o-jcardenass).
- **Bug Fixes & Platform Stability:** Fixed permission request loops on certain head units (#956), removed unreachable modern API calls on legacy Android 4.x devices, resolved mid-drive resolution drops down to 480p, and fixed SurfaceView forced scaling inversion.
